...
class Bird:
    ...
    def draw(self,win):
        ...
        # change bird image from wings up to middle to down to middle to up every 5 ticks then reset
        ...
        # show wings middle image when nose dive
        if self.tilt <= -80:
            self.img = self.IMGS[1]
...
def draw_window(win, birds, pipes, base, score):
    win.blit(BG_IMG,(0,0)) # blit is draw. So draw bg_img at 0,0 i.e top left corner of window
    bird.draw(win)
    for pipe in pipes:
        pipe.draw(win)
